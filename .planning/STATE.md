...
last_updated: "2026-07-04T11:07:00.000Z"
last_activity: 2026-07-04
last_activity_desc: "Completed quick task 260704-oux: Fixed Tauri session persistence by adding the missing capabilities/default.json (grants core:default + store:default). Pending user rebuild + retest."
progress:
  total_phases: 5
  completed_phases: 2
  total_plans: 7
...
- **OPEN (partially resolved 2026-07-04, quick task `260704-jd2`):** Tauri desktop `.exe` still shows the old create-next-app scaffold — it was built before the `260704-h5i` root-page-redirect fix landed on `main`; needs a rebuild (`beforeBuildCommand` re-runs `cd ../web && npm run build`, bundles `apps/web/out`). ~~Additionally, once rebuilt, `apps/native/src-tauri/tauri.conf.json`'s `security.csp` `connect-src` whitelist still only allows `https://macost-api.onrender.com` (the dead Render URL) + Supabase + `localhost:8000`~~ — RESOLVED: CSP now whitelists `https://macost-production.up.railway.app` in both `default-src` and `connect-src` (Render URL fully removed), commit `d31602f`. `apps/web/.env.example` also updated (commit `b4dd64c`) to document that Tauri desktop builds must set `NEXT_PUBLIC_API_BASE_URL=https://macost-production.up.railway.app` at build time. **Still open:** `apps/web/.env.local` (gitignored, per-dev) still has a placeholder `NEXT_PUBLIC_API_BASE_URL=x` and there's no `apps/web/.env.production` — a plain `npm run build` for the Tauri bundle would still inline whatever `.env.local` currently holds unless the developer manually sets it to the Railway URL before building. A full rebuild with the Railway URL baked in, plus verification against the live backend, is still pending.
- ~~Tauri release desktop build shows "Tidak dapat terhubung ke server" on login~~ — ROOT CAUSE FOUND AND FIXED 2026-07-04. After ruling out wrong exe, Windows Firewall, general network/DNS, and (mistakenly, per initial curl tests) CORS, quick task `260704-o31` enabled Tauri's `devtools` Cargo feature (temporary) so DevTools could be opened in the release build. User reproduced the bug with DevTools open and got the exact browser console error: `Access to fetch at 'https://macost-production.up.railway.app/api/auth/login' from origin 'http://tauri.localhost' has been blocked by CORS policy: ... No 'Access-Control-Allow-Origin' header is present`. WebView2 on this Windows setup sends `Origin: http://tauri.localhost` (HTTP), but `backend/main.py`'s CORS `allow_origins` only had `https://tauri.localhost` (HTTPS) — confirmed via curl preflight: `http://tauri.localhost` origin → 400 with no CORS header, `https://tauri.localhost` → 200. Fixed via quick task `260704-ogx` (commit `4fb3c2a`), PR #2 merged to `main`, Railway redeployed — user confirmed (2026-07-04) login now works end-to-end from the Tauri desktop app. Still pending: revert the temporary `devtools` Cargo feature (`260704-o31`) once all Tauri debugging for this session is done, since it shouldn't ship in release builds long-term.
- **OPEN, pending user retest:** Tauri desktop session doesn't persist across app restarts — after logging in and fully closing the app, reopening it requires logging in again (Phase 1 UAT test #3 in `01-UAT.md` had already flagged this as unverified). Root cause found: `apps/native/src-tauri/` had zero `capabilities/` files — in Tauri v2, plugin IPC commands (here, `tauri-plugin-store`'s get/set/save/delete, called from `apps/web/lib/auth/session.ts` to persist the JWT to `.session.dat` via `LazyStore`) require an explicit capability grant per window, and without one the calls are silently rejected. Confirmed directly: no `.session.dat` file existed anywhere on the test machine's filesystem despite a successful login. Fixed via quick task `260704-oux` (commit `b7e9f3c`): added `apps/native/src-tauri/capabilities/default.json` granting `core:default` + `store:default` to the `"main"` window — both permission identifiers confirmed against the real generated `gen/schemas/desktop-schema.json`, and the grant verified to resolve correctly via `cargo check` (Tauri's build-time ACL resolver). **Pending:** a real Tauri rebuild + manual retest (log in, confirm `.session.dat` appears under `%LOCALAPPDATA%\com.zephyra.macost\` or `%APPDATA%\com.zephyra.macost\`, close and reopen the app, confirm it stays logged in) — `cargo check` only proves the grant compiles, not that runtime persistence actually works end-to-end.
