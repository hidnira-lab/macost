...
last_updated: "2026-07-04T10:25:00.000Z"
last_activity: 2026-07-04
last_activity_desc: "Completed quick task 260704-o31: Enabled Tauri devtools (temporary) to debug why the release desktop build still fails to connect to the backend"
progress:
  total_phases: 5
  completed_phases: 2
  total_plans: 7
...
- **OPEN, under active investigation:** Even after rebuilding with the correct Railway URL baked in (both `apps/web/out` and `target/release/macost.exe` binary confirmed via `grep` to contain `https://macost-production.up.railway.app`, 0 occurrences of the dead Render URL) and after `260704-jd2`'s CSP fix, the Tauri **release** desktop build still shows "Tidak dapat terhubung ke server" on login. Ruled out so far: wrong exe (user confirmed running `target/release/macost.exe`, not `debug`), Windows Firewall (no block prompt appeared), general network/DNS (a normal browser on the same machine reaches `https://macost-production.up.railway.app/health` fine), CORS (curl-simulated preflight from both `tauri://localhost` and `https://tauri.localhost` origins returns 200 with correct `access-control-allow-origin`), backend availability (confirmed live). The failure appears specific to the Tauri WebView2 process itself. Quick task `260704-o31` enabled the `devtools` Cargo feature (temporary, should be reverted once root cause is found) so DevTools can be opened in the release build to inspect the actual Console/Network error — this is the next diagnostic step, not yet done.
