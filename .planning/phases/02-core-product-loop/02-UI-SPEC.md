...
**Transaction History (`156:1526`):** **Uses Palette B, not the Palette A tokens used elsewhere in this document** (see the Design System > Color "Palette A/B Discrepancy" note below) — flag for normalization to Palette A during implementation. Header: avatar + "Macost" wordmark + bell. Search bar: white input, `border-[#e5e2e1]`, rounded-8px, search icon left-inset, placeholder "Search transactions..." in `#c0c6d6`. Filter button: 48x48 square, `bg-[#f6f3f2]`, rounded-8px, sliders icon. Date-group headers: small-caps bold `#c0c6d6`, 12px, tracked — "TODAY", "YESTERDAY". Transaction rows are grouped inside ONE bordered container per date (`border-[#f6f3f2]`, rounded-12px) with thin `border-b border-[#f6f3f2]` row dividers — not individually-boxed cards like other lists. Row anatomy: circular category-tinted icon chip + merchant/title (semibold 16px `#1b1b1c`) + category (14px `#414753`) on the left; amount (semibold 16px, `#ba1a1a` "- Rp X" for expense / `#005bb0` "+ Rp X" for income) + time (14px `#717785`) right-aligned. The bottom-nav "History" tab is confirmed as its own destination (resolved — see "Resolved Decisions" above); the active-pill color should use Palette A's `#298dff` per the normalization recommendation rather than this frame's literal `#0074dc`.
